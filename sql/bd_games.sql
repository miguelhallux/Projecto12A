...
-- Inserindo dados na tabela 'produtoras'
...
INSERT INTO `jogos` (`cod`, `nome`, `cod_genero`, `cod_produtora`, `descricao`, `pontuacao`, `capa`) VALUES
...
INSERT INTO `utilizador` (`utilizador`, `nome`, `pass`, `tipo`) VALUES
('admin', 'Mguel Gomes', '$2y$10$UpMQCcir.v649HrdLvUXiOC/ftU7xWhSxm8QhX.VzSe9LZHhvW/Ty', 'admin'),
('teste', 'João da Silva', '$2y$10$w7on7cjLKNtmJUGkiHIXoOQAwTJzkgxXqLmjtfDUkCXmQK0784.IS', 'editor');



--ALTERAÇÕES NAS TABELAS (alterou no )
update generos
SET genero = 'Estratégia'
WHERE cod = 2;
...
-- Seleciona todos os dados da tabela 'jogos'
-- e dá um apelido 'j' para a tabela 'jogos'
SELECT * FROM jogos j 

-- Junta (faz uma ligação) com a tabela 'generos'
-- e dá um apelido 'g' para a tabela 'generos'
-- A junção é feita usando a coluna 'cod_genero' de 'jogos'
-- e a coluna 'cod' de 'generos'
JOIN generos g 

-- A condição para a junção é que 'cod_genero' de 'jogos'
-- deve ser igual a 'cod' de 'generos'
ON j.cod_genero = g.cod;



/* SELECT * FROM jogos j: Isso seleciona todas as colunas da tabela jogos e dá a ela um apelido j. O * significa "todas as colunas".

JOIN generos g: Isso diz que queremos combinar a tabela jogos com a tabela generos, e damos o apelido g para a tabela generos.

ON j.cod_genero = g.cod: Isso define a condição para a junção. Queremos que as linhas da tabela jogos se juntem com as linhas da tabela generos onde o cod_genero de jogos é igual ao cod de generos. */
-- Seleciona algumas colunas específicas da tabela 'jogos' e da tabela 'generos'
-- Damos apelidos 'j' para a tabela 'jogos' e 'g' para a tabela 'generos'


--- ORDENAR A VIEW

SELECT j.cod, j.nome, g.genero, j.descricao,j.pontuacao from jogos j JOIN generos g ON j.cod_genero = g.cod;

SELECT 
    j.cod,          -- Código do jogo
    j.nome,         -- Nome do jogo
    g.genero,  -- Nome do gênero (damos um apelido 'genero' para a coluna 'nome' da tabela 'generos')
    j.descricao,    -- Descrição do jogo
    j.pontuacao     -- Pontuação do jogo
FROM jogos j       -- Seleciona da tabela 'jogos' e dá o apelido 'j'
JOIN generos g     -- Junta com a tabela 'generos' e dá o apelido 'g'
ON j.cod_genero = g.cod;  -- Faz a junção com base no código do gênero




SELECT j.nome,g.genero, p.produtora, j.descricao, j.pontuacao FROM jogos j
JOIN produtoras p ON j.cod_produtora = p.cod
JOIN generos g ON j.cod_genero = g.cod;

-- Seleciona algumas colunas específicas das tabelas 'jogos', 'produtoras' e 'generos'
-- Damos apelidos 'j' para 'jogos', 'p' para 'produtoras' e 'g' para 'generos'
SELECT 
    j.nome,         -- Nome do jogo
    g.nome AS genero,  -- Nome do gênero (damos o apelido 'genero' para a coluna 'nome' da tabela 'generos')
    p.pais,         -- País da produtora
    j.descricao,    -- Descrição do jogo
    j.pontuacao     -- Pontuação do jogo
FROM jogos j       -- Seleciona da tabela 'jogos' e dá o apelido 'j'
JOIN produtoras p  -- Junta com a tabela 'produtoras' e dá o apelido 'p'
ON j.cod_produtora = p.cod  -- Faz a junção com base no código da produtora
JOIN generos g     -- Junta com a tabela 'generos' e dá o apelido 'g'
ON j.cod_genero = g.cod;  -- Faz a junção com base no código do gênero
